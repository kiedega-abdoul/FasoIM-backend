...
class ServicesActivitesTests(
    ...
    def test_demarrer_seance_enregistre_le_debut_reel(self):
        seance = self.creer_seance(formateur=self.formateur)

        with patch(
            "activites.service.timezone.localdate",
            return_value=seance.date_seance,
        ):
            seance = SeanceService.demarrer_seance(
                seance.id, acteur=self.acteur
            )

        self.assertEqual(seance.statut, Seance.Statut.EN_COURS)
        self.assertIsNotNone(seance.date_debut_reelle)
        self.assertIsNone(seance.date_fin_reelle)

    def test_terminer_seance_exige_feuille_cloturee(self):
        seance = self.creer_seance(
            formateur=self.formateur,
            statut=Seance.Statut.EN_COURS,
        )
        seance.date_debut_reelle = timezone.now()
        seance.save(
            update_fields=["date_debut_reelle", "updated_at"]
        )

        with self.assertRaises(ValidationActiviteErreur):
            SeanceService.terminer_seance(
                seance.id, acteur=self.acteur
            )

    def test_terminer_seance_apres_cloture_presence(self):
        seance = self.creer_seance(
            formateur=self.formateur,
            statut=Seance.Statut.EN_COURS,
        )
        ...
        seance = SeanceService.terminer_seance(
            seance.id, acteur=self.acteur
        )

        self.assertEqual(seance.statut, Seance.Statut.TERMINEE)
        self.assertIsNotNone(seance.date_fin_reelle)

    def test_formateur_non_affecte_ne_peut_pas_intervenir(self):
        seance = self.creer_seance(formateur=self.formateur)

        with self.assertRaises(ValidationActiviteErreur):
            SeanceService.exiger_intervenant_seance(
                seance=seance, acteur=self.formateur_2
            )
