...
from accounts.models import AffectationActeur, AffectationRole
...
class SeanceService(ServiceActiviteBase):
    ...
    PERMISSION_DEMARRER = "demarrer_seance"
    PERMISSION_TERMINER = "terminer_seance"


    @staticmethod
    def _est_responsable_centre(*, acteur, session_id, centre_id):
        if getattr(acteur, "is_superuser", False):
            return True

        aujourd_hui = timezone.localdate()
        return AffectationRole.objects.filter(
            affectation_acteur__acteur=acteur,
            affectation_acteur__session_id=session_id,
            affectation_acteur__centre_id=centre_id,
            ...
            | Q(date_expiration__gte=aujourd_hui),
        ).exists()

    @classmethod
    def exiger_intervenant_seance(cls, *, seance, acteur):
        if getattr(acteur, "is_superuser", False):
            return True
        if seance.formateur_id == getattr(acteur, "id", None):
            return True
        if cls._est_responsable_centre(
            acteur=acteur,
            ...
            return True
        raise ValidationActiviteErreur(
            "Seul le formateur affecté ou le Responsable du centre "
            "peut effectuer cette opération sur la séance."
        )
    ...
    def planifier_seance(
        ...
        *,
        ...
        if formateur is None:
            raise ValidationActiviteErreur(
                {"formateur_id": "Le formateur est obligatoire lors de la création de la séance."}
            )
    ...
    @classmethod
    @transaction.atomic
    def demarrer_seance(cls, seance_id, *, acteur):
        seance = SeanceRepository.get_by_id_pour_update(seance_id)
        ControleAccesActivitesService.exiger(
            acteur,
            cls.PERMISSION_DEMARRER,
            session_id=seance.session_id,
            centre_id=seance.centre_id,
        )
        cls.exiger_intervenant_seance(seance=seance, acteur=acteur)
        cls._exiger_session_modifiable(seance.session)
        cls._exiger_activites_actives(seance.session)

        if seance.statut != Seance.Statut.PLANIFIEE:
            raise ValidationActiviteErreur(
                "Seule une séance planifiée peut être démarrée."
            )
        if seance.formateur_id is None:
            raise ValidationActiviteErreur(
                "Un formateur doit être affecté avant le démarrage."
            )
        if timezone.localdate() < seance.date_seance:
            raise ValidationActiviteErreur(
                "La séance ne peut pas être démarrée avant sa date prévue."
            )

        maintenant = timezone.now()
        seance.statut = Seance.Statut.EN_COURS
        seance.date_debut_reelle = maintenant
        seance.date_fin_reelle = None
        return SeanceRepository.sauvegarder(
            seance,
            update_fields=[
                "statut",
    ...
    @classmethod
    @transaction.atomic
    def terminer_seance(cls, seance_id, *, acteur):
        seance = SeanceRepository.get_by_id_pour_update(seance_id)
        ControleAccesActivitesService.exiger(
            acteur,
            cls.PERMISSION_TERMINER,
            session_id=seance.session_id,
            centre_id=seance.centre_id,
        )
        cls.exiger_intervenant_seance(seance=seance, acteur=acteur)
        ...
        if seance.statut != Seance.Statut.EN_COURS:
            raise ValidationActiviteErreur(
                "Seule une séance en cours peut être terminée."
            )
        if (
            seance.statut_feuille_presence
            != Seance.StatutFeuillePresence.CLOTUREE
            ...
            raise ValidationActiviteErreur(
                "La feuille de présence doit être clôturée avant de terminer la séance."
            )
        ...
        if seance.type_seance == Seance.TypeSeance.EVALUATION:
            ...
            if evaluation is None:
                raise ValidationActiviteErreur(
                    "L'évaluation liée à cette séance est introuvable."
                )
            if evaluation.statut != Evaluation.Statut.CLOTUREE:
                raise ValidationActiviteErreur(
                    "L'évaluation doit être clôturée avant de terminer la séance."
                )

        maintenant = timezone.now()
        seance.statut = Seance.Statut.TERMINEE
        seance.date_fin_reelle = maintenant
        return SeanceRepository.sauvegarder(
            seance,
            update_fields=[
                "statut",
...
class NoteService(ServiceActiviteBase):
    ...
    def modifier_note(
        ...
        *,
        ...
        SeanceService.exiger_intervenant_seance(
            seance=note.evaluation.seance, acteur=acteur
        )
